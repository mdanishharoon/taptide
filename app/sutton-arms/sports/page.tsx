...
export default function SuttonArmsSportsPage() {
    return (
        <>
            <SuttonArmsNavbar />
                ...
                <section className="relative pt-32 pb-20 px-6" style={{ backgroundColor: "var(--deep-navy)" }}>
                    <div className="max-w-6xl mx-auto text-center">
                        <motion.h1
                            ...
                            style={{ fontFamily: "var(--font-display)", color: "var(--antique-cream)" }}
                ...
                <section className="py-16 px-6">
                    <div className="max-w-4xl mx-auto">
                        <motion.div className="text-center mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                            <h2 className="text-3xl md:text-4xl mb-4" style={{ fontFamily: "var(--font-display)", color: "var(--deep-navy)" }}>
                                THIS WEEK&apos;S FIXTURES
                            </h2>
                            <p className="text-lg opacity-70" style={{ fontFamily: "var(--font-body)", color: "var(--deep-navy)" }}>
                        ...
                        <div className="space-y-8">
                            {sportsSchedule.map((day, dayIndex) => (
                                <motion.div
                                    ...
                                    style={{ backgroundColor: "#fff", boxShadow: "0 4px 20px rgba(11, 30, 45, 0.1)" }}
                                    initial={{ opacity: 0, y: 30 }}
                                    whileInView={{ opacity: 1, y: 0 }}
                                    viewport={{ once: true }}
                                    transition={{ delay: dayIndex * 0.1 }}
                                >
                                    <div className="px-6 py-4" style={{ backgroundColor: "var(--deep-navy)" }}>
                                        <h3 className="text-xl" style={{ fontFamily: "var(--font-display)", color: "var(--golden-bronze)" }}>
                                            {day.date}
                                        </h3>
                                    </div>
                                    <div className="divide-y" style={{ borderColor: "rgba(11, 30, 45, 0.1)" }}>
                                        {day.events.map((event, eventIndex) => (
                                            <div key={eventIndex} className="px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-gray-50 transition-colors">
                                                <div className="flex items-center gap-4">
                                                    <span className="text-2xl font-bold" style={{ fontFamily: "var(--font-display)", color: "var(--golden-bronze)" }}>
                                                        {event.time}
                                                    </span>
                                                    <div>
                                                        <p className="text-sm uppercase tracking-wider opacity-60" style={{ fontFamily: "var(--font-body)", color: "var(--deep-navy)" }}>
                                                            {event.sport}
                                                        </p>
                                                        <p className="text-lg font-semibold" style={{ fontFamily: "var(--font-body)", color: "var(--deep-navy)" }}>
                                                            {event.match}
                                                        </p>
                                                    </div>
                                                </div>
                                                <span className="px-3 py-1 rounded-full text-sm" style={{ backgroundColor: "var(--ocean-teal)", color: "var(--antique-cream)", fontFamily: "var(--font-body)" }}>
                                                    {event.channel}
                                                </span>
                                            </div>
                                        ))}
                        ...
                        <motion.div className="mt-12 text-center" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
                            <p className="text-lg mb-6 opacity-70" style={{ fontFamily: "var(--font-body)", color: "var(--deep-navy)" }}>
                                Don&apos;t miss a moment of the action!
                            </p>
                            <button
                                className="px-8 py-4 text-sm uppercase tracking-[0.2em] transition-all hover:opacity-90"
                                style={{ fontFamily: "var(--font-body)", backgroundColor: "var(--golden-bronze)", color: "var(--deep-navy)" }}
                            >
                                Book a Table
                            </button>
                        </motion.div>
                ...
                <section className="py-16 px-6" style={{ backgroundColor: "#fff" }}>
                    <div className="max-w-4xl mx-auto">
                        ...
                        <motion.div className="flex justify-center" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
                            <div id="FANZO-widget-container-sa" style={{ maxWidth: "100%", margin: "0 auto" }} />
                            <Script
                                ...
                                onLoad={() => {
                                    if (typeof window !== "undefined") {
                                        ...
                                        if (win.FANZOWidget) {
                                            win.FANZOWidget({ container: "FANZO-widget-container-sa" });
                                        }
                                    }
                                }}
                            />
